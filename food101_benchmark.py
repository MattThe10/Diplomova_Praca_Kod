...
DEFAULT_OUTPUT_CSV = "results/food101_results.csv"
...
def build_deep_base_dataset(data_dir: str) -> Dataset:
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.5450, 0.4435, 0.3436), (0.2302, 0.2409, 0.2387)),
    ])
    train_ds = datasets.Food101(root=data_dir, split="train", download=True, transform=transform)
    test_ds = datasets.Food101(root=data_dir, split="test", download=True, transform=transform)
    return ConcatDataset([train_ds, test_ds])


def build_raw_base_dataset(data_dir: str) -> Dataset:
    train_ds = datasets.Food101(root=data_dir, split="train", download=True)
    test_ds = datasets.Food101(root=data_dir, split="test", download=True)
    return ConcatDataset([train_ds, test_ds])


def infer_num_classes(raw_dataset: Dataset) -> int:
    if hasattr(raw_dataset, "classes"):
        return len(raw_dataset.classes)

    if hasattr(raw_dataset, "dataset") and hasattr(raw_dataset.dataset, "classes"):
        return len(raw_dataset.dataset.classes)
...
def train_deep_model(
    ...
    return RunResult(
        ...
        notes="deep_model; no_resize=True; batch_padding=True",
    )
...
def evaluate_classical_scores(
    ...
    expanded_scores = np.full(
        (decision_scores.shape[0], len(all_classes)),
        -1e9,
        dtype=np.float32
    )
    ...
    for j, cls in enumerate(model_classes):
        expanded_scores[:, class_to_index[int(cls)]] = decision_scores[:, j]

    return compute_classification_metrics(y_true, expanded_scores, len(all_classes))
...
def train_sift_model(
    ...
    model_classes = clf.named_steps["svm"].classes_

    val_scores = clf.decision_function(x_val_bovw)
    val_top1, val_top5, val_macro_ap = evaluate_classical_scores(
        y_val,
        val_scores,
        model_classes,
        all_classes,
    ...
    infer_time = time.perf_counter() - infer_start
    test_top1, test_top5, test_macro_ap = evaluate_classical_scores(
        y_test,
        test_scores,
        model_classes,
        all_classes,
...
def train_hog_model(
    ...
    print("[hog] starting scaler + svm fit...")
    clf.fit(x_train_hog, y_train)
    print("[hog] svm fit done")
    svm_train_time = time.perf_counter() - svm_start
    train_time = time.perf_counter() - train_start
...
def parse_args():
    ...
    parser.add_argument("--val-count", type=int, default=15000)
    parser.add_argument("--test-count", type=int, default=15000)
